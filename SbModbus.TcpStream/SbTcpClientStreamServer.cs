...
/// </summary>
public class SbTcpClientStreamServer : IModbusStreamServer
{
  ...
  /// <summary>
  ///   获取底层 TCP 客户端流（用于直接访问连接配置）
  /// </summary>
  public SbTcpClientStream Stream { get; }
  ...
  public void Start()
  {
    if (_isDisposed) throw new SbModbusException("TCP client server stream has been disposed");
  ...
  /// <summary>
  ///   TCP 连接状态变化 → 翻译为会话事件。
  ///   TCP client 内置自动重连，被动断连后重连成功会再次触发此回调。
  /// </summary>
