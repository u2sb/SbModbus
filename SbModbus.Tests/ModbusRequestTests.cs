...
public class ModbusRequestTests
{
    ...
    {
        var req = ModbusRequest.CreateReadCoils(0, 8, 1);
    ...
    {
        var req = ModbusRequest.CreateReadHoldingRegisters(100, 5, 1);
    ...
    {
        var req = ModbusRequest.CreateReadInputRegisters(200, 3, 1);
    ...
    {
        var req = ModbusRequest.CreateWriteSingleCoil(5, true, 1);
    ...
    {
        var req = ModbusRequest.CreateWriteSingleCoil(5, false, 1);
        Assert.Equal(ModbusFunctionCode.WriteSingleCoil, req.FunctionCode);
    ...
    public void CreateWriteSingleRegister_WithBytes_ValidatesCorrectly()
    {
        ...
        var req = ModbusRequest.CreateWriteSingleRegister(7, data, 1);
    ...
    {
        var req = ModbusRequest.CreateWriteSingleRegister(7, 0x1234, true, 1);
        Assert.Equal(ModbusFunctionCode.WriteSingleRegister, req.FunctionCode);
        Assert.Equal((ushort)7, req.StartingAddress);
        // 大端模式: MSB 在前
        Assert.Equal(0x12, req.Data[0]);
    ...
    {
        var req = ModbusRequest.CreateWriteSingleRegister(7, 0x1234, false, 1);
        // 小端模式: LSB 在前
    ...
    public void CreateWriteMultipleRegisters_ValidatesCorrectly()
    {
        ...
        var req = ModbusRequest.CreateWriteMultipleRegisters(20, data, 1);
    ...
    {
        var bits = new System.Collections.BitArray(new bool[] { true, false, true, true, false, true, false, false });
        var req = ModbusRequest.CreateWriteMultipleCoils(30, bits, 1);
    ...
    public void CreateReadRequest_InvalidQuantity_Zero_Throws()
    {
        Assert.Throws<SbModbusException>(() =>
            ModbusRequest.CreateReadRequest(ModbusFunctionCode.ReadCoils, 0, 0, 1));
    }
    ...
    public void CreateReadRequest_OutOfRange_Throws()
    {
        // 地址 + 数量 > 65536
        Assert.Throws<SbModbusException>(() =>
            ModbusRequest.CreateReadRequest(ModbusFunctionCode.ReadCoils, 65500, 100, 1));
    }
    ...
    public void CreateWriteRequest_InvalidDataLength_Throws()
    {
        // 写单个线圈需要 2 字节数据
        Assert.Throws<SbModbusException>(() =>
            ModbusRequest.CreateWriteRequest(ModbusFunctionCode.WriteSingleCoil, 0, [0xFF], 1, 1));
    }
    ...
    public void Reset_ResetsResponseToReady()
    {
        ...
        req.Response.Data = new byte[] { 0x01 };
