...
public class IntToStringConverter : IValueConverter
{
  ...
  {
    // 处理null值
    if (value == null) return parameter as string ?? string.Empty;

    // int转string
    if (value is int intValue) return intValue.ToString(culture ?? CultureInfo.CurrentCulture);
  ...
  public object? ConvertBack(object? value, Type targetType, object? parameter, CultureInfo culture)
  {
    ...
    if (value is string stringValue)
      ...
      {
        if (targetType == typeof(int?)) return result;
    ...
    try
    {
      ...
      if (targetType == typeof(int?)) return converted;
